    ...
    <section class="px-0 py-10 md:py-16">
        <div class="mx-auto w-[min(1180px,calc(100%_-_32px))] md:w-[min(1180px,calc(100%_-_40px))]">
            <div class="mb-8 max-w-3xl md:mb-10">
                <p class="font-handwritten m-0 text-2xl font-semibold text-blue-600">Portofolio</p>
                <h1 class="font-display my-2 text-3xl font-bold leading-tight tracking-tight md:text-5xl">{{ $heading }}</h1>
                        ...
                        <div class="p-5">
                            ...
                            <div class="mt-4 flex flex-wrap gap-2">
                                @foreach (array_filter(array_map('trim', explode(',', $project['tags']))) as $tag)
                                    <span class="font-code rounded-full bg-blue-50 px-2.5 py-1 text-[10px] font-medium text-blue-700 dark:bg-blue-600/15 dark:text-blue-200">{{ $tag }}</span>
                                @endforeach
                            </div>
